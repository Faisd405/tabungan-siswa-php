      ...
      <div class="main-content">
        <section class="section">
          ...
          <?php elseif ($_SESSION['user']['role'] == 'siswa') : ?>
            <?php
            $query = "SELECT * FROM students WHERE user_id = {$_SESSION['user']['id']}";
            $student = $conn->query($query)->fetch_assoc();

            $query = "SELECT * FROM savings_accounts WHERE student_id = {$student['id']}";
            $saving_account = $conn->query($query)->fetch_assoc();

            $query = "SELECT SUM(amount) as total FROM transactions WHERE account_id = {$saving_account['id']}";
            $transactions = $conn->query($query)->fetch_assoc();

            $query = "SELECT COUNT(*) as total FROM transactions WHERE account_id = {$saving_account['id']}";
            $total_transactions = $conn->query($query)->fetch_assoc();

            $query = "SELECT * FROM transactions WHERE account_id = {$saving_account['id']} ORDER BY id DESC LIMIT 5";
            $data = $conn->query($query);
            ?>
              ...
              <div class="col-lg-4 col-md-4 col-sm-12">
                <div class="card card-statistic-2">
                  ...
                  <div class="card-wrap">
                    ...
                    <div class="card-body">
                      IDR <?php echo number_format($saving_account['balance'], 2, ',', '.'); ?>
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-lg-4 col-md-4 col-sm-12">
                <div class="card card-statistic-2">
                  ...
                  <div class="card-wrap">
                    ...
                    <div class="card-body">
                      <?php echo $total_transactions['total']; ?>
                    </div>
                  </div>
                </div>
              </div>
